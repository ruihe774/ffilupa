...
def test_LuaObject_operators():
    ...
    for op in "+ - * / // % ** & | ^ << >> == < <= > >= !=".split():
        if (
            SpecifierSet("<5.3").contains(lua.lualib.version)
            and op in "& | ^ << >> //".split()
        ):
            continue
        try:
...
def test_not_impl():
    with pytest.raises(
        TypeError,
        match=r"^unsupported operand type\(s\) for \+: 'LuaTable' and 'LuaTable'$",
    ):
        lua.table() + lua.table()
...
def test_table_operators():
    ...
    tb = lua.table_from(
        dict(
            zip(
                itertools.count(1),
                "the quick brown fox jumps over the lazy doges".split(),
            )
        )
    )
    del tb[3]
    assert sorted(tb.items(), key=lambda o: o[0]) == sorted(
        zip(itertools.count(1), "the quick fox jumps over the lazy doges".split()),
        key=lambda o: o[0],
    )


def test_table_iter_and_abc():
    tb = lua.table(the="quick", brown="fox", jumps="over", lazy="doges")
    ...
    assert sorted(it) == sorted(
        zip("the brown jumps lazy".split(), "quick fox over doges".split())
    )
    assert "lazy" in tb
...
def test_lua_string():
    s = lua._G.python.to_luaobject(
        b"the quick brown fox jumps over the lazy doges".replace(b" ", b"\0")
    )
    assert bytes(s) == b"the quick brown fox jumps over the lazy doges".replace(
        b" ", b"\0"
    )
