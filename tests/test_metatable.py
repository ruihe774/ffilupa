...
def test_simple_operators():
    for name, op in ops.items():
        if (
            ...
            and name in "band bor bxor bnot shl shr idiv".split()
        ):
            continue
        if isinstance(op, binary_op):
            ...
            status, exception = lua.eval(
                "pcall(function() return a{}b end)".format(op.lua_op)
            )
            assert status == False
            ...
            status, exception = lua.eval(
                "pcall(function() return {}a end)".format(op.lua_op)
            )
            assert status == False
            assert isinstance(exception, TypeError)
...
def test_method_call():
    ...
    assert lua.execute(
        "local a, b, c = python.table_arg(awd.c)({1, a2=2}); return a == awd and b == 1 and c == 2"
    )
    assert lua.execute(
        "local a, b, c = python.table_arg(Awd.c)({awd, 1, a2=2}); return a == awd and b == 1 and c == 2"
    )
    assert lua.eval('awd.d() == "awd"')
    assert lua.eval('Awd.d() == "awd"')
    assert lua.eval("awd.e(awd) == awd")
    assert lua.execute(
        "local b, c = python.table_arg(awd.f)({1, a2=2}); return b == 1 and c == 2"
    )
    assert lua.execute(
        "local b, c = python.table_arg(Awd.f)({1, a2=2}); return b == 1 and c == 2"
    )
    ...
    assert lua.execute(
        "local a, b, c = python.table_arg(awd.i)({1, a2=2}); return a == Awd and b == 1 and c == 2"
    )
    assert lua.execute(
        "local a, b, c = python.table_arg(Awd.i)({1, a2=2}); return a == Awd and b == 1 and c == 2"
    )
...
def test_namedtuple_newindex():
    class tp:
        __slots__ = ("YaeSakura", "RaidenMei", "布洛妮娅", "德丽莎")
