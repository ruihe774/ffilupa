...
def test_as_attrgetter():
    assert (
        lua.eval('python.as_attrgetter(python.to_dict({awd="dwa"})):pop("awd")')
        == "dwa"
    )
...
def test_as_itemgetter():
    assert (
        lua.eval(
            'python.as_itemgetter(python.as_attrgetter(python.to_dict({awd="dwa"}))).awd'
        )
        == "dwa"
    )
...
def test_to_str():
    ...
    with LuaRuntime(autodecode=False) as lua_na:
        assert lua_na.eval('python.to_bytes("稳态")'.encode("gb18030")) == "稳态".encode(
            "gb18030"
        )
...
def test_to_set():
    assert lua.eval('python.to_set({2, 2, 3, awd="dwa", def="ccc"})') == {
        2,
        3,
        "dwa",
        "ccc",
...
def test_to_dict():
    assert lua.eval('python.to_dict({2, 2, 3, awd="dwa", def="ccc"})') == {
        1: 2,
        2: 2,
        3: 3,
        "awd": "dwa",
